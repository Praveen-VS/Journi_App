...
export default function RegisterPage() {
  ...
  const getPasswordStrength = () => {
    if (!password) return { level: 0, label: '', color: 'bg-neutral-200' };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!agreed) {
      setErrorText('Please accept the Terms of Service & Privacy Policy.');
      return;
    }
    setErrorText('');
    setIsLoading(true);
  ...
  return (
    <div className="w-full min-h-screen bg-[#FFF5F8] dark:bg-[#1A030C] flex flex-col md:items-center md:justify-center md:p-6 lg:p-10">
      ...
      <div className="w-full max-w-5xl md:rounded-[36px] overflow-hidden md:shadow-2xl bg-white dark:bg-[#240612] md:border border-[#FF4F7A]/15 grid grid-cols-1 md:grid-cols-2 relative">
        ...
        <div className="relative w-full h-[360px] sm:h-[400px] md:h-auto min-h-[380px] md:min-h-[680px] flex flex-col justify-between p-6 sm:p-8 overflow-hidden">
          {/* Background Image: Sunset traveler from onboarding */}
          <div className="absolute inset-0 -z-10">
            <Image
              src="/images/onboarding/m04-save-bg.png"
              alt="Journi Save Journey"
              fill
              priority
              className="object-cover object-top md:object-center"
            />
            <div className="absolute inset-0 bg-gradient-to-b from-black/25 via-transparent to-black/45 md:to-transparent" />
          </div>
          ...
          <div className="z-10 mt-6 md:mt-10">
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-black text-[#5B0B24] tracking-tight leading-tight drop-shadow-sm">
              Begin Your
              <br />
              <span className="text-[#E61E50]">Journey</span>
            </h1>
            <p className="text-sm sm:text-base text-[#5B0B24]/90 font-medium mt-2 max-w-xs drop-shadow-sm">
              Craft unforgettable travel stories with AI simplicity.
            </p>
          </div>

          {/* Bottom Left Accent badge */}
          ...
          <div>
            ...
            <div className="mb-5">
              ...
              <p className="text-xs sm:text-sm text-[#704250] dark:text-[#FFB3C6] font-medium mt-1">
                Start planning with intelligent AI recommendations.
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-3.5">
              ...
              {password && (
                <div className="px-1 space-y-1">
                  <div className="flex justify-between items-center text-[11px] font-semibold text-[#704250]">
                    <span>Password Strength</span>
                    <span className={strength.level >= 2 ? 'text-emerald-600' : 'text-amber-600'}>
                      {strength.label}
                    </span>
                  </div>
                  <div className="h-1.5 w-full bg-[#E8DFE3] rounded-full overflow-hidden flex gap-1">
              ...
              {/* Terms Checkbox */}
              <div className="pt-1">
                <label className="flex items-start gap-2.5 cursor-pointer select-none text-xs text-[#5B0B24] dark:text-[#FFB3C6]">
                  <input
                    type="checkbox"
                    checked={agreed}
                    onChange={(e) => setAgreed(e.target.checked)}
                    className="w-4 h-4 mt-0.5 rounded border-[#D8C7CF] text-[#E61E50] focus:ring-[#E61E50] accent-[#E61E50]"
                  />
                  <span>
                    I agree to the{' '}
                    <Link href="/terms" className="font-semibold text-[#E61E50] hover:underline">
                      Terms of Service
                    </Link>{' '}
                    and{' '}
                    <Link href="/privacy" className="font-semibold text-[#E61E50] hover:underline">
                      Privacy Policy
                    </Link>
                  </span>
                </label>
              ...
              {/* Primary Gradient CTA Button: Create Account -> */}
              <button
                type="submit"
                disabled={isLoading}
            ...
            <div className="grid grid-cols-3 gap-2.5 sm:gap-3">
              <button
                ...
                className="flex flex-col items-center justify-center p-2.5 rounded-2xl border border-[#E8DFE3] dark:border-[#FF4F7A]/20 hover:border-[#E61E50] hover:bg-[#FFF5F8] dark:hover:bg-[#2D0A18] transition-all text-center group"
